...
RES=../results
export PROBE_OPEN_TIME=15   # gate-open time before the probe is withdrawn (Eq. 13: ~1.5 % bound)
REPS=${1:-5}
TRUN=50
M0=0.70
...
FEXT=0     # identification demo: no wind, identical conditions for passive and active
MINSAMP=2600
...
for rep in $(seq 1 $REPS); do run_one probe   "$rep" "probe probe_az=0.00 probe_al=0.80"; done
