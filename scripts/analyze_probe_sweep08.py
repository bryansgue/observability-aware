...
ax1.axhline(0.009, color="k", ls="--", lw=1); ax1.text(len(x)-0.6, 0.0105, r"$\tilde\sigma_{\min}$", fontsize=7, ha="right")
ax1.set_ylabel(r"$\tilde\sigma$ (probe)", color="#1f77b4"); ax1.set_ylim(0, 0.06)
ax1.set_xticks(x); ax1.set_xticklabels(labels, fontsize=7); ax1.set_xlabel("vertical/lateral mix at budget $\\rho=0.8$ m")
ax2 = ax1.twinx()
t2_m = [np.nanmean(res[t][1]) for t, _ in SHAPES]; t2_s = [np.nanstd(res[t][1]) for t, _ in SHAPES]
ax2.errorbar(x, t2_m, t2_s, fmt="o-", color="#c0392b", capsize=2, ms=4, lw=1.2, label="time to 2 % error")
ax2.set_ylabel("time to 2 % error [s]", color="#c0392b"); ax2.set_ylim(0, 18); ax2.grid(False)
h1, l1 = ax1.get_legend_handles_labels(); h2, l2 = ax2.get_legend_handles_labels()
ax1.legend(h1+h2, l1+l2, loc="upper center", bbox_to_anchor=(0.5, 1.2), ncol=2, frameon=False, fontsize=7)
fig.savefig(os.path.join(FIGS, "fig_probe_opt.png"), dpi=300)
