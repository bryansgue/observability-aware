...
def main():
    ...
    fig, axs = plt.subplots(3, 1, figsize=(7.0, 6.6))
    ...
    for k, (ax, (sw, xlab)) in enumerate(zip(axs, sweeps)):
        ...
        ax.plot(xs, fa, "o-", color="#1f77b4", ms=5, lw=1.6, label="force error [N]")
        ax.set_xlabel(xlab); ax.set_ylabel("[N]", color="#1f77b4")
        ax.tick_params(axis="y", labelcolor="#1f77b4")
        ax2 = ax.twinx()
        ax2.plot(xs, me, "s--", color="#c0392b", ms=5, lw=1.6, label="mass error [%]")
        ax2.set_ylabel("[%]", color="#c0392b")
