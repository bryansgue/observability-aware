...
\title{Observability-Aware Virtual Force Sensing and
Self-Calibration for Quadrotors}

\author{Bryan~S.~Guevara, Jove~Varela-Aldas, Angelica~Carrion,
and~Tiago~Nacimiento}
...
the onboard IMU into a \emph{virtual force sensor}: the external specific force is
read directly from the accelerometer residual. The obstacle is calibration, since
the reading needs the vehicle mass, which is unobservable near hover, where it is
indistinguishable from a vertical force; a standard extended Kalman filter (EKF)
then corrupts the mass. We make the sensor \emph{observability-aware and
self-calibrating}. An online identifiability measure, the Fisher information on the
mass (its inverse is the Cram\'er--Rao bound), gates the calibration and freezes the
mass when it cannot be seen; and when the flight is too gentle to recalibrate, the
controller injects a minimal excitation that the same measure shapes. In simulation
the sensor reads the external force to about \SI{0.4}{\newton} RMS per axis (per-axis
correlation \num{0.93} under continuous wind); the gate holds the mass to
\SI{3}{\percent} in a windy hover where a plain EKF drifts to \SI{7}{\percent}; the
probe recovers the mass from a wrong prior that hover cannot identify; and feeding
the sensed force to a model-predictive controller cuts the tracking error by up to
\SI{80}{\percent} under wind. The sensor degrades gracefully under injected
accelerometer and thrust errors and runs in real time at \SI{100}{\hertz}.
\end{abstract}

\begin{IEEEkeywords}
Virtual sensor, force and wrench estimation, inertial sensing, self-calibration,
observability, active sensing, Cram\'er--Rao bound, aerial robots.
\end{IEEEkeywords}

% ════════════════════ INTRODUCTION ════════════════════
\section{Introduction}\label{sec:intro}

The external force acting on a quadrotor, from wind, a payload, or physical
...
to know \emph{when} its calibration is trustworthy and to be able to recalibrate
...
trustworthy or to \emph{act} so that it becomes observable. In near-hover flight
...
gentle to reveal it, the model-predictive controller injects a minimal probe that
...
\caption{Architecture of the self-calibrating virtual force sensor. The EKF outputs
the external force $\hat{\vect{d}}$, the mass calibration $\hat m$, and the
identifiability $\tilde\sigma$, which gates the calibration and triggers the active
probe; the NMPC (application) feeds $\hat{\vect{d}}$ forward.}
\label{fig:system}
\end{figure}
...
virtual force sensor trustworthy on a vehicle whose calibration parameter is only
conditionally identifiable.
\begin{itemize}
\item We characterize the IMU-residual reading as a virtual force sensor that needs
no added hardware and no differentiation of position; it matches the simulator
ground truth to about \SI{0.4}{\newton} RMS per axis (a per-axis correlation of
\num{0.93} under continuous wind), and a
moving-horizon estimator does not improve on it because the reading is algebraically
observable.
...
protected during hover; and when the mass must be recalibrated but the flight is
too gentle, the controller injects a probe to restore identifiability. The same
$\tilde\sigma$ \emph{shapes} the probe, since maximizing it is the
most informative excitation, so a single observability scalar governs gating,
triggering, and excitation design.
\item As an application, we use the sensor in closed loop: feeding the sensed force
to a model-predictive controller reduces the tracking error by up to
\SI{80}{\percent} under wind, demonstrating the virtual sensor in real time at
\SI{100}{\hertz}.
\end{itemize}
...
also absorbs the aerodynamic drag. We treat $\vect{d}$ as quasi-static over the
...
absorbed aerodynamic drag, the former dominating near hover and the latter growing
...
nonsingular, so $\vect{d}$ is recovered whenever the window is nonempty. The
...
case at hover. Its dimension is thrust squared (\si{\newton\squared}), so its
...
hover. The horizontal components $d_x$ and $d_y$ remain observable regardless of
excitation, which is why the external force is recovered in the directions
...
$N\mat{I}_3$, is read with the same accuracy in every regime. The mass is thus
...
Table~\ref{tab:obs}. The external force is recoverable in every regime and is
...
force. This motivates the two elements of our method: an estimator that trusts
the mass only when $\sigma$ is large (Section~\ref{sec:mhe}), and a controller
that creates the excitation when the mass must be identified but the flight is
too gentle to reveal it (Section~\ref{sec:nmpc}).
...
never gated, since $\vect{d}$ is observable throughout (Section~\ref{sec:obs}).
...
cancels the external force; the rate loop uses the nominal time constant
$\tau_{rc}$. The prediction model carries no explicit drag term; because the
estimator runs with $c=0$, the aerodynamic drag is absorbed into $\hat{\vect{d}}$
and is therefore compensated through the same feedforward. The disturbance
estimate is smoothed by an exponential moving average before injection.
...
the attitude error through the quaternion logarithm, with the same hemisphere
correction as the estimator, and the input is regularized around the hover
...
with $T_{\max}=5mg$ and $\omega_{\max}=\SI{20}{\radian\per\second}$.
...
information on the mass, so the probe that maximizes it (equivalently, its
dimensionless form $\tilde\sigma$ at a fixed thrust budget) per unit deviation
...
identifies the two levers: a vertical bob of amplitude $a_z$ that modulates the
thrust magnitude $T$, and a lateral motion of amplitude $a_\ell$ that tilts the
thrust axis $\vect{a}$. Both disperse the regressor $T\vect{a}$, so the probe is
\begin{equation}
\vect{p}^{\text{ref}}\!\leftarrow\!\vect{p}^{\text{ref}}\!+\!
\big[a_\ell(\cos\omega_\ell t\!-\!1),\,a_\ell\sin\omega_\ell t,\,a_z\sin\omega_z t\big]^{\!\top},
\;\; \tilde\sigma<\tilde\sigma_{\min},
\label{eq:probe}
\end{equation}
and we choose the $(a_z,a_\ell)$ mix that maximizes $\tilde\sigma$ subject to a
fixed displacement budget $a_z^2+a_\ell^2\le\rho^2$, identified empirically in
...
Drag              & $c$ (estimator)            & $0$ (absorbed in $\hat{\vect{d}}$) \\
...
Active probe      & budget $\rho$, $f$, optimal & \SI{0.40}{\meter}, \SI{0.5}{\hertz}, lateral \\
NMPC              & $N_c$, horizon             & $31$, \SI{1.5}{\second} \\
NMPC weights      & $\mat{Q}_p,\mat{Q}_a,\mat{Q}_v$ & $180,\ 20,\ 30$ \\
                  & $\mat{R}_u$                & $\mathrm{diag}(0.3,5,5,5)$ \\
NMPC limits       & $T_{\max},\,\omega_{\max}$  & $5mg$, \SI{20}{\radian\per\second} \\
\bottomrule
\end{tabular}
\end{table}
...
$\sim$\SI{2}{\newton} disturbance; the step correlation is \num{0.91} to
\num{0.98} across speeds (inflated by the step transitions, hence reported
alongside the absolute error); and under a sinusoidal wind the correlation is
\num{0.93} (Section~\ref{sec:robust}), the most demanding case. A moving-horizon
...
\caption{Virtual force sensor in the time domain (single representative run): the
EKF estimate (the specific force $\hat{\vect{d}}$ scaled by the mass to newtons)
tracks the applied wind steps on all three axes.}
\label{fig:dhat}
\end{figure}
...
All axes read accurately ($0.91$--$0.98$, step ground truth); $d_z$ stays high in
hover because the held mass absorbs the $m$--$d_z$ ambiguity of
Proposition~\ref{prop:hover}.}
\label{fig:force}
\end{figure}
...
\caption{Calibration curve of the virtual force sensor: estimated force
$m\hat{\vect{d}}$ versus the applied force, per axis, under a sinusoidal wind. The
near-unity slope and high $R^2$ show the IMU-residual reading is a well-calibrated
force measurement.}
\label{fig:calib}
\end{figure}
...
\caption{Mass identification under excitation. From a wrong prior
(\SI{0.60}{\kilogram}) the EKF converges to within \SI{0.6}{\percent} of the
ground truth on a disturbance-free trajectory.}
\label{fig:massconv}
\end{figure}
...
\caption{Mass observability against excitation, probed with an \emph{ungated} EKF
($N{=}10$ per speed, $N{=}9$ in hover; mean $\pm$ std): biased in hover, accurate
under excitation, confirming Proposition~\ref{prop:hover} and the $\sigma^{-1}$
conditioning.}
\label{fig:mass}
\end{figure}
...
\caption{Mass over a windy maneuver-to-hover transition ($N{=}5$, mean $\pm$ std).
In the hover (shaded) the plain EKF drifts to $\sim$\SI{7}{\percent} and the MHE to
$\sim$\SI{5}{\percent}, while the obs-aware EKF (ours) holds \SI{3}{\percent} once
$\tilde\sigma$ falls below the threshold (bottom).}
\label{fig:trans}
\end{figure}
...
mean $\pm$ std. Passively the estimate stays frozen; the active probe (shaded) lifts
$\tilde\sigma$ above the threshold (bottom), recovers the mass, then withdraws.}
\label{fig:active}
\end{figure}
...
at a fixed displacement budget, from a pure lateral tilt to a pure vertical bob, and
...
tightly distributed, as \eqref{eq:crb} predicts: it is a variance bound, so the
highest-$\tilde\sigma$ probe yields the lowest-variance recovery. The most informative
mix (\probeOptShape) recovers the mass to \SI{\probeOptErr}{\percent}, against
\SI{\probeFixErr}{\percent} for the original mixed shape at the same displacement;
...
\caption{Probe shaping by identifiability. Sweeping the probe's vertical/lateral mix
at a fixed displacement budget, the mass recovery error falls as $\tilde\sigma$
rises; the $\tilde\sigma$-maximizing mix is circled.}
\label{fig:probeopt}
\end{figure}
...
\caption{Trajectory under wind, orthogonal projections sharing the $x$ axis. With
the estimated-force feedforward (blue) the vehicle stays on the reference
(dotted); without it (red) the wind pushes it off the path.}
\label{fig:traj}
\end{figure}
...
\caption{Disturbance rejection against speed ($N=5$ per mode; hover $N=4$, mean $\pm$ std). Feeding the
estimated force to the controller reduces the tracking error most when the wind
dominates (hover) and less as the speed grows.}
\label{fig:rej}
\end{figure}

\begin{table}[t]
\centering
\caption{EKF estimation against trajectory speed ($N=10$): mass estimate and
...
alike: the probe sustains $\tilde\sigma$ above the threshold (the lateral tilt
reaching the highest value of the probe family, Fig.~\ref{fig:probeopt}), so it
reliably re-opens the gate while hover keeps it closed.
...
\caption{Virtual force sensor under a sinusoidal wind (single representative run), a
different disturbance profile from the steps of Fig.~\ref{fig:dhat}. The estimate
($\hat{\vect{d}}$ scaled to newtons) tracks the ground truth on all three axes
...
\caption{Robustness to real-sensor errors. Force absolute error (blue, left axis)
and mass calibration error (red, right axis) against injected accelerometer bias,
accelerometer scale-factor error, and thrust-coefficient mismatch. The virtual
sensor degrades gracefully and proportionally, with no catastrophic failure.}
\label{fig:robust}
\end{figure}
...
the vertical force, which it absorbs, and drifts in hover (Fig.~\ref{fig:trans}),
...
\begin{table}[t]
\centering
\caption{Estimator comparison on identical data. Force and excited-mass entries are
pooled over the battery ($N{=}10$); hover and recovery entries are from
Figs.~\ref{fig:trans} and~\ref{fig:active}.}
\label{tab:compare}
\begin{tabular}{lccc}
\toprule
...
when the mass must be recalibrated but the flight is too gentle. In
...
force feedforward reduces the tracking error by up to \SI{80}{\percent} under
wind.
...
\emph{IEEE Robotics Autom.\ Lett.}, vol.~3, no.~1, pp.~60--67, 2018. arXiv:1711.03896.
