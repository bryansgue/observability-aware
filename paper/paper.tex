...
\author{Jos\'e~Varela-Ald\'as and Ang\'elica~Carri\'on}
...
observers~\cite{tomic2017external,ruggiero2014impedance,yuksel2014nonlinear}, which estimate the
...
estimate~\cite{smeur2016adaptive,hanover2022performance,chen2016disturbance}, and learning-based adaptive controllers fit a
...
convention of~\cite{shuster1993survey}. The body rate $\vect{\omega}\in\R^3$ is
...
the window is $\tfrac{\mat{G}}{r_s}$ with $\mat{G}$ the Gramian \eqref{eq:gramian}. The
$(1,1)$ block of its inverse is the inverse Schur complement, $[\mat{G}^{-1}]_{11}=\sigma^{-1}$,
so the \emph{marginal} Fisher information of the inverse mass $\beta$, after
profiling out the jointly estimated force $\vect{d}$, is $\tfrac{\sigma}{r_s}$, and the
...
estimation-theoretic meaning: $\tfrac{\sigma}{r_s}$ is the marginal Fisher information on the
...
scaling $\tfrac{\hat m}{m}$, whose error is bounded by \eqref{eq:crb} where the mass is
