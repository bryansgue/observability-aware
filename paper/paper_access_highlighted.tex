...
%DIF ADD paper_access.tex                Sun Aug 30 15:49:18 2026
\usepackage{cite}
...
parameter it cannot see. \DIFdelbegin \DIFdel{Fig.}\DIFdelend \DIFaddbegin \DIFadd{Figure}\DIFaddend ~\ref{fig:system} shows the resulting loop and the role of the
...
\DIFdelbegin \DIFdel{Fig.}\DIFdelend \DIFaddbegin \DIFadd{Figure}\DIFaddend ~\ref{fig:dhat} shows the estimate tracking the disturbance in the time
...
\centering
\includegraphics[width=\textwidth]{figs/fig_calib.png}
\caption{Calibration curve under a sinusoidal wind: estimated force against
applied force for the (a) $x$, (b) $y$, and (c) $z$ axes, with the linear fit
(slopes \num{0.98}, \num{0.97}, \num{0.96}; $R^2$ \num{0.90}, \num{0.89},
...
accurately. \DIFdelbegin \DIFdel{Fig.}\DIFdelend \DIFaddbegin \DIFadd{Figure}\DIFaddend ~\ref{fig:massconv} shows the EKF converging from a deliberately
...
filter. \DIFdelbegin \DIFdel{Fig.}\DIFdelend \DIFaddbegin \DIFadd{Figure}\DIFaddend ~\ref{fig:mass} shows the plain EKF mass against speed under wind
...
\DIFdelbegin \DIFdel{Fig.}\DIFdelend \DIFaddbegin \DIFadd{Figure}\DIFaddend ~\ref{fig:trans} flies an aggressive trajectory followed by a hover, both
...
\DIFdelbegin \DIFdel{Fig.}\DIFdelend \DIFaddbegin \DIFadd{Figure}\DIFaddend ~\ref{fig:mass} (\SI{4.8}{\percent}), because the maneuver leaves the filter
...
recover it. \DIFdelbegin \DIFdel{Fig.}\DIFdelend \DIFaddbegin \DIFadd{Figure}\DIFaddend ~\ref{fig:active} starts from a deliberately wrong prior
...
\SI{1}{\percent} }\hskip0pt%DIFAUXCMD
($0.4\pm0.2$}\DIFaddend , $N{=}5$)\DIFdelbegin \DIFdel{; the recovery }\DIFdelend \DIFaddbegin \DIFadd{, the same
accuracy the filter reaches on the disturbance-free trajectory of
Figure~\ref{fig:massconv} (\mbox{%DIFAUXCMD
...
}\DIFaddend depends on the probe shape, which the next experiment \DIFdelbegin \DIFdel{optimizes.
By contrast, under a disturbance-free trajectory the same filter
identifies the mass to within \mbox{%DIFAUXCMD
\SI{0.6}{\percent} }\hskip0pt%DIFAUXCMD
(Fig.~\ref{fig:massconv}); the
residual here is the price of identifying in hover from a wrong prior.
}\DIFdelend \DIFaddbegin \DIFadd{examines.
}\DIFaddend 
...
\caption{Mass recovery from a wrong hover prior (\SI{0.70}{\kilogram}; wind-free
hover; $N{=}5$, mean $\pm$ std): the probe (shaded) lifts $\tilde\sigma$, recovers
the mass, and is withdrawn after \SI{15}{\second} of gate-open time; the estimate
is then held for the rest of the hover.}
\label{fig:active}
\end{figure}
\DIFaddend 
...
\SI{1}{\milli\second} \DIFdelbegin \DIFdel{. Fig.}\DIFdelend \DIFaddbegin \DIFadd{on the laptop-class processor of Section~\ref{sec:nmpc}. Figure}\DIFaddend ~\ref{fig:traj} shows the spatial effect on the
...
to its parameters. Figure~\ref{fig:fisher}(a) plots, against speed, the across-run
...
so the margin narrows. }\emph{\DIFadd{Maneuver intensity}}\DIFadd{: at peak speeds of
\mbox{%DIFAUXCMD
...
\DIFdelend \DIFaddbegin \url{https://github.com/bryansgue/observability-aware}\DIFaddend .
