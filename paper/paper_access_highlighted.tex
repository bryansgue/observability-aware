...
%DIF DEL submitted_v1/paper_access.tex   Sat Aug 29 13:09:48 2026
%DIF ADD paper_access.tex                Tue Sep  1 20:03:24 2026
\usepackage{cite}
...
%DIF < % Filled from the probe sweep (scripts/analyze_probe_sweep.py).
...
\long\def\DIFadd#1{{\color{blue}#1}} %DIF PREAMBLE
\long\def\DIFdel#1{} %DIF PREAMBLE
\def\DIFaddbegin{\color{blue}}\def\DIFaddend{\color{black}}\def\DIFdelbegin{}\def\DIFdelend{} %DIF PREAMBLE
\long\def\DIFaddFL#1{{\color{blue}#1}} %DIF PREAMBLE
\long\def\DIFdelFL#1{} %DIF PREAMBLE
\def\DIFaddbeginFL{\color{blue}}\def\DIFaddendFL{\color{black}}\def\DIFdelbeginFL{}\def\DIFdelendFL{} %DIF PREAMBLE
...
\DIFdelbegin %DIFDELCMD < \title{Observability-Aware Virtual Force Sensing and Self-Calibration for Quadrotors}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \title{Identifiability-Aware Virtual Force Sensing and Self-Calibration for Quadrotors}
\DIFaddend 
...
\markboth
{J. Varela-Ald\'as \headeretal:~\DIFdelbegin \DIFdel{Observability-Aware }\DIFdelend \DIFaddbegin \DIFadd{Identifiability-Aware }\DIFaddend Virtual Force Sensing and Self-Calibration for Quadrotors}
{J. Varela-Ald\'as \headeretal:~\DIFdelbegin \DIFdel{Observability-Aware }\DIFdelend \DIFaddbegin \DIFadd{Identifiability-Aware }\DIFaddend Virtual Force Sensing and Self-Calibration for Quadrotors}
...
\DIFdelbegin \DIFdel{unobservable }\DIFdelend \DIFaddbegin \DIFadd{unidentifiable }\DIFaddend near hover, where \DIFdelbegin \DIFdel{it }\DIFdelend \DIFaddbegin \DIFadd{a mass error }\DIFaddend is indistinguishable from a vertical force; a
standard extended Kalman filter then corrupts the mass. We make the sensor
\emph{\DIFdelbegin \DIFdel{observability-aware }\DIFdelend \DIFaddbegin \DIFadd{identifiability-aware }\DIFaddend and self-calibrating}. An online identifiability
...
\begin{keywords}
Aerial robotics, force estimation, inertial sensing, \DIFdelbegin \DIFdel{observability}\DIFdelend \DIFaddbegin \DIFadd{identifiability}\DIFaddend ,
...
hardware, and the reading is instantaneous: no \DIFdelbegin \DIFdel{double-differentiation }\DIFdelend \DIFaddbegin \DIFadd{double differentiation }\DIFaddend of position,
no estimator lag.
...
\emph{conditionally identifiable}: \DIFaddbegin \DIFadd{the mass enters the reading only through the
thrust-to-weight ratio, so }\DIFaddend in near-hover flight \DIFdelbegin \DIFdel{it }\DIFdelend \DIFaddbegin \DIFadd{a mass error }\DIFaddend is indistinguishable
from a vertical external force, \DIFdelbegin \DIFdel{so }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend an estimator that calibrates it unconditionally
...
consistency constraints~\cite{wuest2019online,wensing2017geometric}\DIFaddbegin \DIFadd{, and with
sparse-regression models identified from flight data for predictive
control~\mbox{%DIFAUXCMD
\cite{guevara2025sindy}}\hskip0pt%DIFAUXCMD
}\DIFaddend ; these
...
of dead-zone and covariance-management schemes~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{smeur2016adaptive}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{ioannou1996robust,narendra1987persistent}}\hskip0pt%DIFAUXCMD
}\DIFaddend , and
...
controller's trigger to restore \DIFdelbegin \DIFdel{observability.
}\DIFdelend \DIFaddbegin \DIFadd{identifiability.
...
provide it. Extended-state and disturbance observers~\mbox{%DIFAUXCMD
\cite{han2009pid,chen2016disturbance}
}\hskip0pt%DIFAUXCMD
...
trusted and acts when it cannot; dead-zone and covariance-management schemes~\mbox{%DIFAUXCMD
\cite{ioannou1996robust,narendra1987persistent}
}\hskip0pt%DIFAUXCMD
...
\cite{mehra1974optimal,morelli2022inertia} }\hskip0pt%DIFAUXCMD
and dual
control~\mbox{%DIFAUXCMD
\cite{mesbah2018dual} }\hskip0pt%DIFAUXCMD
plan the excitation offline or implicitly in the
cost; and recursive inertial identification~\mbox{%DIFAUXCMD
\cite{wuest2019online,burri2018system,boyacioglu2023inertial}
}\hskip0pt%DIFAUXCMD
...
}\DIFaddend Second, the external force and the inertial parameters are coupled, so estimating
...
reliable or to \emph{act} so that it becomes \DIFdelbegin \DIFdel{observable}\DIFdelend \DIFaddbegin \DIFadd{identifiable}\DIFaddend . In near-hover flight
the mass and the vertical external force enter the dynamics through the same
channel and are not jointly \DIFdelbegin \DIFdel{observable}\DIFdelend \DIFaddbegin \DIFadd{identifiable}\DIFaddend , so an estimator that updates both
distributes the error between them. This is the property we exploit.

This paper builds an \DIFdelbegin \DIFdel{observability-aware}\DIFdelend \DIFaddbegin \DIFadd{identifiability-aware}\DIFaddend , self-calibrating virtual force sensor.
...
make the calibration \DIFdelbegin \DIFdel{observability-aware }\DIFdelend \DIFaddbegin \DIFadd{identifiability-aware }\DIFaddend on two levels. An extended Kalman filter
(EKF) monitors an online identifiability measure and freezes the mass calibration
when the parameter is \DIFdelbegin \DIFdel{unobservable}\DIFdelend \DIFaddbegin \DIFadd{unidentifiable}\DIFaddend , protecting the reading during hover where a
...
\DIFdelbegin \DIFdel{probe }\DIFdelend \DIFaddbegin \emph{\DIFadd{probe}}\DIFadd{, a small bounded excursion of the position reference (not a sensor),
}\DIFaddend that restores identifiability and then stops, so the sensor never relies on a
parameter it cannot see. \DIFdelbegin \DIFdel{Fig.~\ref{fig:system} shows the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend resulting loop and the role of the identifiability
signal $\tilde\sigma$ \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{are shown in Fig.~\ref{fig:system}.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{figure}[t]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=\columnwidth]{figs/fig_system.pdf}
%DIFDELCMD < \caption{Architecture of the self-calibrating virtual force sensor: the
%DIFDELCMD < identifiability $\tilde\sigma$ gates the mass calibration and triggers the
%DIFDELCMD < active probe; the NMPC feeds $\hat{\vect{d}}$ forward.}
%DIFDELCMD < \label{fig:system}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{table*}[t]\color{blue}
\centering
...
\DIFaddbegin \begin{figure}[t]\color{blue}
\centering
...
\DIFaddend The contributions of this work are the following. The bare reading of the external
...
contribution is the \DIFdelbegin \DIFdel{observability-aware }\DIFdelend \DIFaddbegin \DIFadd{identifiability-aware }\DIFaddend self-calibration layer that makes such a
virtual force sensor reliable on a vehicle whose calibration parameter is only
conditionally identifiable.
\begin{itemize}
\item We give the \DIFdelbegin \DIFdel{observability }\DIFdelend \DIFaddbegin \DIFadd{identifiability }\DIFaddend analysis of the sensor's calibration: the mass and
the vertical external force are not jointly \DIFdelbegin \DIFdel{observable }\DIFdelend \DIFaddbegin \DIFadd{identifiable }\DIFaddend in near-hover flight, and we
...
calibration by this measure, \DIFdelbegin \DIFdel{freezing it when unobservable }\DIFdelend \DIFaddbegin \DIFadd{with an explicit quantitative criterion, the
...
unidentifiable }\DIFaddend so the reading is protected during hover; and when the mass must be recalibrated but the flight
...
most informative excitation within the probe family, so a single \DIFdelbegin \DIFdel{observability
}\DIFdelend \DIFaddbegin \DIFadd{identifiability
}\DIFaddend scalar governs gating, triggering, and excitation design.
\end{itemize}
...
algebraically \DIFdelbegin \DIFdel{observable}\DIFdelend \DIFaddbegin \DIFadd{identifiable}\DIFaddend , and feeding the sensed force to a model-predictive
controller reduces the tracking error by \SI{34}{} to \SI{80}{\percent} under
...
errors to probe the dominant \DIFdelbegin \DIFdel{real-sensor effects }\DIFdelend \DIFaddbegin \DIFadd{effects of a real sensor}\DIFaddend , but a full hardware demonstration,
on a real IMU and thrust map, or on a manipulator whose joint-torque readings give
the same \DIFdelbegin \DIFdel{observability }\DIFdelend \DIFaddbegin \DIFadd{identifiability }\DIFaddend structure, is the natural next step and is deliberately left
...
Section~\ref{sec:obs} analyzes \DIFdelbegin \DIFdel{observability }\DIFdelend \DIFaddbegin \DIFadd{identifiability }\DIFaddend and derives the hover ambiguity and
the identifiability measure. Section~\ref{sec:mhe} describes the
\DIFdelbegin \DIFdel{observability-aware }\DIFdelend \DIFaddbegin \DIFadd{identifiability-aware }\DIFaddend EKF and its mass gate. Section~\ref{sec:nmpc} presents the
disturbance-rejecting controller and the active excitation.
Section~\ref{sec:setup} reports the experimental setup and
Section~\ref{sec:results} the results. Section~\ref{sec:conclusion} concludes \DIFaddbegin \DIFadd{the paper}\DIFaddend .
...
convention of~\cite{shuster1993survey}. The body \DIFdelbegin \DIFdel{rate }\DIFdelend \DIFaddbegin \DIFadd{attitude rate (body rate for short) }\DIFaddend $\vect{\omega}\in\R^3$ is
...
negligible and \DIFaddbegin \DIFadd{the weight of }\DIFaddend an added mass and a vertical force become indistinguishable
...
nominal known constant of the inner loop, not estimated. \DIFaddbegin \DIFadd{To be explicit about
...
whose error is characterized in Section~\ref{sec:senserr} (a \mbox{%DIFAUXCMD
\SI{10}{\percent}
}\hskip0pt%DIFAUXCMD
...
%DIF <  ════════════════════ OBSERVABILITY ════════════════════
\DIFdelbegin %DIFDELCMD < \section{Observability and Excitation}%%%
\DIFdelend %DIF >  ════════════════════ IDENTIFIABILITY ════════════════════
\DIFaddbegin \section{Identifiability and Excitation}\DIFaddend \label{sec:obs}
...
The \DIFaddbegin \DIFadd{same distinction separates the reading from an energy-based one. With all
...
\DIFadd{The }\DIFaddend estimator augments the position and velocity with the mass and the force.
...
full nonlinear observability test of the augmented state. \DIFaddbegin \DIFadd{We therefore speak of
}\emph{\DIFadd{identifiability}} \DIFadd{throughout: the property in question is that of the
...
parameters of a rigid body can be found in~\mbox{%DIFAUXCMD
\cite{boyacioglu2023inertial}}\hskip0pt%DIFAUXCMD
. }\DIFaddend This
identifiability determines when the mass and the force can be separated \DIFdelbegin \DIFdel{, }\DIFdelend and is
...
nonsingular, so given the mass\DIFaddbegin \DIFadd{, }\DIFaddend $\vect{d}$ is recovered whenever the
...
\label{eq:schur}
\end{equation}
The scalar $\sigma$ is the dispersion of the \DIFdelbegin \DIFdel{vectors }\DIFdelend \DIFaddbegin \DIFadd{thrust }\emph{\DIFadd{vectors}} \DIFaddend $T_k\vect{a}_k$
about their mean\DIFaddbegin \DIFadd{, and it becomes positive through a change in the thrust
magnitude $T_k$, a change in its direction $\vect{a}_k$, or both}\DIFaddend ; it is zero only
when \DIFdelbegin \DIFdel{they }\DIFdelend \DIFaddbegin \DIFadd{the vectors }\DIFaddend are identical across the window, which is the case at hover. Its dimension is thrust squared (newtons squared\DIFdelbegin \DIFdel{, not to be
confused with the window length }\DIFdelend \DIFaddbegin \DIFadd{; the italic }\DIFaddend $N$ \DIFaddbegin \DIFadd{denotes the window length}\DIFaddend ), so its
absolute scale grows with the thrust level of the maneuver. To obtain a
dimensionless \DIFdelbegin \DIFdel{, maneuver-comparable measure}\DIFdelend \DIFaddbegin \DIFadd{measure, comparable across maneuvers, }\DIFaddend we normalize it by the
thrust energy in the window,
\begin{equation}
\tilde\sigma \triangleq \frac{\sigma}{\sum_k T_k^2}\in[0,1],
\label{eq:snorm}
\end{equation}
the \DIFdelbegin \DIFdel{relative dispersion }\DIFdelend \DIFaddbegin \DIFadd{fraction }\DIFaddend of the thrust \DIFdelbegin \DIFdel{direction}\DIFdelend \DIFaddbegin \DIFadd{energy that lies in the dispersion}\DIFaddend : $\tilde\sigma=0$
at hover and $\tilde\sigma\!\to\!1$ when the thrust \DIFdelbegin \DIFdel{vector is }\DIFdelend \DIFaddbegin \DIFadd{vectors are }\DIFaddend maximally spread.
\DIFdelbegin \DIFdel{Dividing out
}\DIFdelend \DIFaddbegin \DIFadd{Normalizing by }\DIFaddend the absolute thrust level removes the dominant source of the
...
\DIFdelbegin \DIFdel{, as Section~\ref{sec:results}
quantifies}\DIFdelend \DIFaddbegin \DIFadd{(Section~\ref{sec:sens}). The normalization also discards information that the
...
dimensionless }\emph{\DIFadd{classification}} \DIFadd{signal that decides the regime, and
$\sigma/r_s$ is the }\emph{\DIFadd{information content}} \DIFadd{that lower-bounds the variance;
Section~\ref{sec:sens} reports both side by side for every regime flown}\DIFaddend .
...
\DIFdelbegin \DIFdel{observable}\DIFdelend \DIFaddbegin \DIFadd{identifiable}\DIFaddend , while $\beta$ and the vertical force $d_z$ are not separately
\DIFdelbegin \DIFdel{observable}\DIFdelend \DIFaddbegin \DIFadd{identifiable}\DIFaddend .
\end{proposition}
\begin{proof}
...
\DIFdelbegin \DIFdel{observable }\DIFdelend \DIFaddbegin \DIFadd{identifiable }\DIFaddend while $\beta$ and $d_z$ collapse onto their sum.
\end{proof}
...
\DIFdelbegin \DIFdel{observable }\DIFdelend \DIFaddbegin \DIFadd{identifiable }\DIFaddend there, which is why the external force is recovered in the directions
...
\DIFdelbegin \DIFdel{observable }\DIFdelend \DIFaddbegin \DIFadd{identifiable }\DIFaddend under motion and degenerate only at hover, which
...
no extra sensing. It also has an estimation-theoretic meaning\DIFdelbegin \DIFdel{. With accelerometer noise of variance $r_s$, the }\DIFdelend \DIFaddbegin \DIFadd{, which we state as
...
\DIFadd{Let the accelerometer noise be zero-mean with isotropic covariance $r_s\mat{I}_3$
...
\end{proposition}
\begin{proof}
\DIFadd{The }\DIFaddend joint Fisher information of $(\beta,\vect{d})$ over the window is
\DIFdelbegin \DIFdel{$\tfrac{\mat{G}}{r_s}$ }\DIFdelend \DIFaddbegin \DIFadd{$\mat{G}/r_s$, }\DIFaddend with $\mat{G}$ the Gramian \eqref{eq:gramian}. \DIFdelbegin \DIFdel{The
}\DIFdelend \DIFaddbegin \DIFadd{By the block-inverse
formula, the }\DIFaddend $(1,1)$ \DIFdelbegin \DIFdel{block of its inverse }\DIFdelend \DIFaddbegin \DIFadd{entry of $\mat{G}^{-1}$ }\DIFaddend is the inverse \DIFdelbegin \DIFdel{Schur complement, }\DIFdelend \DIFaddbegin \DIFadd{of the Schur complement
of the force block, }\DIFaddend $[\mat{G}^{-1}]_{11}=\sigma^{-1}$\DIFdelbegin \DIFdel{,
so the }\emph{\DIFdel{marginal}} %DIFAUXCMD
\DIFdel{Fisher information of the inverse mass $\beta$, after
profiling out the jointly estimated force $\vect{d}$, is $\tfrac{\sigma}{r_s}$}\DIFdelend , and the Cram\'er--Rao
\DIFdelbegin \DIFdel{bound on any unbiased estimate is
}%DIFDELCMD < \begin{equation}
%DIFDELCMD < \mathrm{var}(\hat\beta)\ \geq\ r_s\,[\mat{G}^{-1}]_{11}\,=\,r_s\,\sigma^{-1},
%DIFDELCMD < \label{eq:crb}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{inequality gives \eqref{eq:crb}.
}\end{proof}

\DIFadd{The bound }\DIFaddend diverges as $\sigma\to0$ at hover and tightens as the maneuver disperses
the thrust \DIFdelbegin \DIFdel{vector. The bound \eqref{eq:crb} }\DIFdelend \DIFaddbegin \DIFadd{vectors. It }\DIFaddend is the batch least-squares \DIFdelbegin \DIFdel{CRB under zero-mean accelerometer noise of isotropic covariance $r_s\mat{I}_3$ and a
quasi-static $\vect{d}$ over the window; it is the limit }\DIFdelend \DIFaddbegin \DIFadd{bound under }\DIFaddend the \DIFaddbegin \DIFadd{stated
assumptions, the limit that the }\DIFaddend recursive filter approaches under excitation, \DIFdelbegin \DIFdel{used here }\DIFdelend \DIFaddbegin \DIFadd{and
we use it }\DIFaddend as a relative identifiability measure rather than an absolute
uncertainty. \DIFdelbegin \DIFdel{Equation~\eqref{eq:crb} gives the }\emph{\DIFdel{raw}} %DIFAUXCMD
\DIFdel{scalar $\sigma$ an
estimation-theoretic meaning: $\tfrac{\sigma}{r_s}$ is the }\DIFdelend \DIFaddbegin \DIFadd{It is stated on the inverse mass $\beta$; since
...
\DIFadd{Proposition~\ref{prop:crb} holds $\vect{d}$ constant over the window, whereas the
...
trajectory rather than a single vector, and the exact }\DIFaddend marginal Fisher
information on \DIFdelbegin \DIFdel{the
mass, so }\DIFdelend \DIFaddbegin \DIFadd{$\beta$ is a weighted Schur complement
$\sigma_{\mathrm{eff}}\leq\sigma$, derived at the end of this section, which
reduces to }\DIFaddend $\sigma$ \DIFdelbegin \DIFdel{is the inverse Cram\'er--Rao bound (up to the noise variance).
The bound is
stated on the inverse mass }\DIFdelend \DIFaddbegin \DIFadd{as $q_d\to0$. Section~\ref{sec:sens} evaluates it on logged
flight data: for the implemented $q_d=10^{-2}$, a drift of about
\mbox{%DIFAUXCMD
\SI{1}{\meter\per\square\second} }\hskip0pt%DIFAUXCMD
per second (half the applied wind), the
information in a \mbox{%DIFAUXCMD
\SI{1}{\second} }\hskip0pt%DIFAUXCMD
window is \mbox{%DIFAUXCMD
\SI{12}{\percent} }\hskip0pt%DIFAUXCMD
...
\emph{\DIFadd{Exact information under the random-walk force model.}} \DIFadd{Stack the compensated specific forces of the window as
...
\label{eq:rwmodel}
\end{equation}
\DIFadd{with $[\mat{C}]_{kl}=\min(k,l)-1$, where the increments have been marginalized into
...
\end{bmatrix},
\end{equation}
\DIFadd{and the marginal information on }\DIFaddend $\beta$ \DIFdelbegin \DIFdel{; since
$\mathrm{var}(\hat m)\approx m^4\,\mathrm{var}(\hat\beta)$ for small errors,
the
bound on the mass itself scales with the same $\sigma^{-1}$.
}\DIFdelend \DIFaddbegin \DIFadd{is its Schur complement,
}\begin{equation}
\begin{aligned}
...
\label{eq:sigeff}
\end{equation}
\DIFaddend For \DIFaddbegin \DIFadd{$q_d=0$, $\mat{\Sigma}=r_s\mat{I}_{3N}$ and \eqref{eq:sigeff} reduces to
...
about \mbox{%DIFAUXCMD
\SI{0.7}{\milli\second} }\hskip0pt%DIFAUXCMD
at $N=100$ when evaluated numerically, so it could replace $\sigma$ in the
gate; since the gate thresholds the dimensionless ratio with margin, we retain the
closed-form $\tilde\sigma$.
}

\DIFadd{For }\DIFaddend \emph{gating} we use \DIFdelbegin \DIFdel{its }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend dimensionless normalization $\tilde\sigma$
\eqref{eq:snorm}, which \DIFdelbegin \DIFdel{divides out }\DIFdelend \DIFaddbegin \DIFadd{normalizes by }\DIFaddend the thrust level so that a single threshold
$\tilde\sigma_{\min}$ holds across maneuvers; $\sigma$ carries the information
content and $\tilde\sigma$ carries the decision. \DIFdelbegin \DIFdel{We do not invert \eqref{eq:crb} to
set $\tilde\sigma_{\min}$ analytically; it is tuned }\DIFdelend \DIFaddbegin \DIFadd{The threshold is set }\DIFaddend with margin
between the hover and maneuver values of $\tilde\sigma$ (Section~\DIFdelbegin \DIFdel{\ref{sec:results}}\DIFdelend \DIFaddbegin \DIFadd{\ref{sec:sens}}\DIFaddend ),
and \DIFdelbegin \DIFdel{\eqref{eq:crb}
reports the uncertainty that any admissible thresholdtolerates. }\DIFdelend \DIFaddbegin \DIFadd{Proposition~\ref{prop:crb} translates it into the best resolution that the
...
\label{eq:thrcrb}
\end{equation}
\DIFadd{With the parameters used in the experiments ($\tilde\sigma_{\min}=0.009$,
$N=100$, $r_s=0.5$; Section~\ref{sec:setup}) this is \mbox{%DIFAUXCMD
\SI{7.6}{\percent} }\hskip0pt%DIFAUXCMD
for one second of data and
\mbox{%DIFAUXCMD
\SI{2.4}{\percent} }\hskip0pt%DIFAUXCMD
after ten. The bound is a floor, not a guarantee: for a
one-second window at or below the threshold, even an efficient unbiased
estimator could not attain a standard deviation smaller than \mbox{%DIFAUXCMD
\SI{7.6}{\percent}}\hskip0pt%DIFAUXCMD
,
which is of the scale of the \mbox{%DIFAUXCMD
\SIrange{5}{7}{\percent} }\hskip0pt%DIFAUXCMD
hover bias the gate guards
against (Section~\ref{sec:results}). Under the same ideal constant-force
assumptions, the floor crosses the \mbox{%DIFAUXCMD
\SI{3.3}{\percent} }\hskip0pt%DIFAUXCMD
residual observed in the
windy hover after approximately \mbox{%DIFAUXCMD
\SI{5.3}{\second} }\hskip0pt%DIFAUXCMD
of accumulated admissible data
and reaches \mbox{%DIFAUXCMD
\SI{2.4}{\percent} }\hskip0pt%DIFAUXCMD
after ten seconds. These figures characterize only
the best-case variance permitted by the information; they neither predict nor
upper-bound the realized estimation error. }\DIFaddend The estimator uses
...
%DIFDELCMD < \centering
%DIFDELCMD < \caption{Observability regimes (Proposition~\ref{prop:hover}). The ``degraded''
%DIFDELCMD < $d_z$ entry is the joint case; with an accurate held mass it is recovered
%DIFDELCMD < (Section~\ref{sec:results}).}
%DIFDELCMD < \label{tab:obs}
%DIFDELCMD < \begin{tabular}{lcc}
%DIFDELCMD < \toprule
%DIFDELCMD < Quantity & Hover & Maneuver \\
%DIFDELCMD < \midrule
%DIFDELCMD < mass $m$        & unobservable ($\sigma\!\approx\!0$)         & observable ($\sigma$ large) \\
%DIFDELCMD < $d_x,\,d_y$     & observable                                  & observable \\
%DIFDELCMD < $d_z$           & degraded ($m$--$d_z$)                       & observable \\
...
% ════════════════════ ESTIMATOR ════════════════════
\DIFdelbegin %DIFDELCMD < \section{Observability-Aware Estimation}%%%
\DIFdelend \DIFaddbegin \section{Identifiability-Aware Estimation}\DIFaddend \label{sec:mhe}
...
\DIFaddbegin \begin{table}[t]\color{blue}
\centering
...
\DIFaddend \subsection{Prediction and update}
...
\DIFaddbegin \subsection{Validity of the linearization}\label{sec:linear}
\DIFadd{The EKF linearizes the model around the current estimate, so it is worth
...
with a relative curvature error of $(\delta m/m)^2$: below \mbox{%DIFAUXCMD
\SI{1}{\percent} }\hskip0pt%DIFAUXCMD
for a
\mbox{%DIFAUXCMD
\SI{10}{\percent} }\hskip0pt%DIFAUXCMD
mass error, and \mbox{%DIFAUXCMD
\SI{18}{\percent} }\hskip0pt%DIFAUXCMD
for the deliberately wrong
\mbox{%DIFAUXCMD
\SI{0.60}{\kilogram} }\hskip0pt%DIFAUXCMD
prior of the identification experiment of Section~\ref{sec:results}, which the filter still
...
(\mbox{%DIFAUXCMD
\SI{0.5}{\meter\per\square\second} }\hskip0pt%DIFAUXCMD
at $\omega=\SI{5}{\radian\per\second}$ and
...
\DIFaddend \subsection{The mass gate}\label{sec:gate}
A plain EKF updates every state at every step, including the mass when it is
\DIFdelbegin \DIFdel{unobservable}\DIFdelend \DIFaddbegin \DIFadd{unidentifiable}\DIFaddend . Under the hover ambiguity of Proposition~\ref{prop:hover} this
attributes a vertical force to a mass change and corrupts the estimate. The
\DIFdelbegin \DIFdel{observability-aware }\DIFdelend \DIFaddbegin \DIFadd{identifiability-aware }\DIFaddend filter instead conditions the mass update on the
identifiability $\tilde\sigma$ of \eqref{eq:snorm}, computed online over a sliding
window from the known thrust and attitude\DIFaddbegin \DIFadd{; the mass is declared unidentifiable,
quantitatively, when $\tilde\sigma$ falls below the threshold $\tilde\sigma_{\min}$
(Table~\ref{tab:params})}\DIFaddend . When the mass is not persistently
...
never gated, since, given the held mass, $\vect{d}$ is \DIFdelbegin \DIFdel{observable }\DIFdelend \DIFaddbegin \DIFadd{identifiable }\DIFaddend throughout
(Section~\ref{sec:obs}).
...
and place the contribution in the \DIFdelbegin \DIFdel{observability }\DIFdelend \DIFaddbegin \DIFadd{identifiability }\DIFaddend logic rather than in the
estimator class.
...
\DIFaddbegin \DIFadd{The smoothing is not a deficiency of the estimator but a separation of roles:
the filter's $q_d$ is set for the bandwidth of the force reading, so its output
retains sample-to-sample noise at \mbox{%DIFAUXCMD
\SI{100}{\hertz} }\hskip0pt%DIFAUXCMD
that the controller would
otherwise pass straight into the thrust command; the moving average
($\alpha_d=0.92$, a time constant of \mbox{%DIFAUXCMD
\SI{0.12}{\second}}\hskip0pt%DIFAUXCMD
) is a low-pass on the
actuation path that can be tuned independently of the estimation bandwidth.
The mass is smoothed more heavily ($\alpha_m=0.995$, \mbox{%DIFAUXCMD
\SI{2}{\second}}\hskip0pt%DIFAUXCMD
) because it
sets the hover equilibrium of the prediction model, and a step in it would step
the commanded thrust. }\DIFaddend In the
...
%DIFDELCMD < J=\ &\sum_{k=0}^{N_c-1}\Big(
...
weighted by $\mat{Q}_N=\mathrm{diag}(\mat{Q}_p,\mat{Q}_v,\mat{Q}_a)$. The \DIFaddbegin \DIFadd{terms of \eqref{eq:nmpc-cost} carry different units, and the weights are their
normalization: with the values of Table~\ref{tab:params}, a position error of
\mbox{%DIFAUXCMD
\SI{0.1}{\meter}}\hskip0pt%DIFAUXCMD
, a velocity error of \mbox{%DIFAUXCMD
\SI{0.25}{\meter\per\second}}\hskip0pt%DIFAUXCMD
, an attitude
error of \mbox{%DIFAUXCMD
\SI{0.3}{\radian}}\hskip0pt%DIFAUXCMD
, and a thrust deviation of \mbox{%DIFAUXCMD
\SI{2.5}{\newton} }\hskip0pt%DIFAUXCMD
each
contribute about \mbox{%DIFAUXCMD
\num{1.8} }\hskip0pt%DIFAUXCMD
to the stage cost, so no single term dominates at
the errors the controller operates at. The }\DIFaddend inputs
are bounded by
$0\leq T\leq T_{\max}$ and $\norm{\vect{\omega}_{\text{cmd}}}_\infty\leq\omega_{\max}$,
with $T_{\max}=\SI{49}{\newton}$ ($\approx4.8mg$) and
$\omega_{\max}=\SI{20}{\radian\per\second}$. \DIFaddbegin \DIFadd{The control law is the receding-horizon
...
\label{eq:rhc}
\end{equation}
\DIFadd{after which the horizon shifts by one step and the problem is solved again.
}\DIFaddend 
...
per-step solve time below \SI{1}{\milli\second}. \DIFaddbegin \DIFadd{All timings in this paper are
...
in sustained hover the mass stays \DIFdelbegin \DIFdel{unobservable }\DIFdelend \DIFaddbegin \DIFadd{unidentifiable }\DIFaddend and the estimate is frozen at
...
\DIFdelbegin \DIFdel{unobservable }\DIFdelend \DIFaddbegin \DIFadd{unidentifiable }\DIFaddend direction. The same identifiability that gates the estimate now
...
\DIFdelbegin \DIFdel{minimizes the mass uncertainty}\DIFdelend \DIFaddbegin \DIFadd{maximizes the corresponding best-case information on the mass}\DIFaddend . We do not solve the continuous
optimal-input-design problem~\cite{mehra1974optimal}\DIFaddbegin \DIFadd{, nor the flight-test
input design of~\mbox{%DIFAUXCMD
\cite{morelli2022inertia}}\hskip0pt%DIFAUXCMD
}\DIFaddend ; instead we maximize
...
%DIFDELCMD < \vect{p}^{\text{ref}}\!\leftarrow\!\vect{p}^{\text{ref}}\!+\!
%DIFDELCMD < \big[\delta_\ell(\cos\omega_\ell t\!-\!1),\,\delta_\ell\sin\omega_\ell t,\,\delta_z\sin\omega_z t\big]^{\!\top},
%DIFDELCMD < \;\; \tilde\sigma<\tilde\sigma_{\min},
%DIFDELCMD < \label{eq:probe}
%DIFDELCMD < \end{equation}%%%
...
probe is triggered only while the mass is both needed and \DIFdelbegin \DIFdel{unobservable; once the estimate has converged it is withdrawn and the gate }\DIFdelend \DIFaddbegin \DIFadd{unidentifiable, and it
...
\mbox{%DIFAUXCMD
\SI{1}{\percent}}\hskip0pt%DIFAUXCMD
, the order of the residual observed. The gate then }\DIFaddend again
protects the mass. This is the active
counterpart of the gate.

\subsection{Remarks on closed-loop behavior}
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{In the evaluated runs, the }\DIFaddend model error reaching the controller \DIFdelbegin \DIFdel{stays bounded}\DIFdelend \DIFaddbegin \DIFadd{remains small}\DIFaddend .
The uncompensated force is the estimation residual
$\vect{d}-\hat{\vect{d}}$, \DIFdelbegin \DIFdel{small because $\vect{d}$ is
observable throughout, }\DIFdelend and the mass enters only as the multiplicative thrust
scaling $\tfrac{\hat m}{m}$\DIFdelbegin \DIFdel{, whose error is bounded by \eqref{eq:crb} }\DIFdelend \DIFaddbegin \DIFadd{. Equation~\eqref{eq:crb} lower-bounds the variance
attainable }\DIFaddend where the mass is \DIFdelbegin \DIFdel{observable and frozen at its identification value }\DIFdelend \DIFaddbegin \DIFadd{identifiable; }\DIFaddend where it is not\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, the gate freezes the
mass at its last identified value. Neither statement is an upper bound on the
realized estimation error. }\DIFaddend A formal stability \DIFaddbegin \DIFadd{or boundedness }\DIFaddend analysis of the
...
estimator, controller, and the scripts that reproduce every figure \DIFdelbegin \DIFdel{will be
released on publication}\DIFdelend \DIFaddbegin \DIFadd{are provided
in the repository given at the end of the paper}\DIFaddend , together with a data dictionary that maps each logged
...
\DIFdelbegin %DIFDELCMD < \begin{table}[t]
%DIFDELCMD < \centering
%DIFDELCMD < \caption{Estimator and controller parameters.}
%DIFDELCMD < \label{tab:params}
%DIFDELCMD < \setlength{\tabcolsep}{4pt}
%DIFDELCMD < \resizebox{\columnwidth}{!}{%
%DIFDELCMD < \begin{tabular}{lll}
%DIFDELCMD < \toprule
%DIFDELCMD < Block & Parameter & Value \\
%DIFDELCMD < \midrule
%DIFDELCMD < Plant / sim       & mass, rate $\tau_{rc}^{-1}$    & \SI{1.05}{\kilogram}, \SI{18}{\per\second} \\
%DIFDELCMD < Drag              & $c$ (estimator)            & $0$ (lumped into $\hat{\vect{d}}$) \\
%DIFDELCMD < Odometry noise    & injected on $p,v$          & MuJoCo, released config \\
%DIFDELCMD < Wind              & profile, amplitude         & step / sine, $\sim$\SI{2}{\newton} \\
%DIFDELCMD < EKF meas.\ var.   & $r_p,\,r_v,\,r_s$           & $0.02,\ 0.1,\ 0.5$ \\
%DIFDELCMD < EKF process       & $q_p,\,q_v,\,q_m,\,q_d$     & $10^{-6},10^{-3},10^{-5},10^{-2}$ \\
%DIFDELCMD < Mass gate         & window, dwell $t_d$, $\tilde\sigma_{\min}$ & \SI{1.0}{\second}, \SI{0.25}{\second}, $0.009$ \\
%DIFDELCMD < Smoothing         & $\alpha_m,\,\alpha_d$       & $0.995,\ 0.92$ \\
%DIFDELCMD < Active probe      & budget $\rho$, $f_\ell$, $f_z$, optimal & \SI{0.40}{\meter}, \SI{0.5}{\hertz}, \SI{1.0}{\hertz}, lateral \\
%DIFDELCMD < NMPC              & $N_c$, horizon             & $31$, \SI{1.5}{\second} \\
%DIFDELCMD < NMPC weights      & $\mat{Q}_p,\mat{Q}_a,\mat{Q}_v$ & $180,\ 20,\ 30$ \\
%DIFDELCMD <                   & $\mat{R}_u$                & $\mathrm{diag}(0.3,5,5,5)$ \\
%DIFDELCMD < NMPC limits       & $T_{\max},\,\omega_{\max}$  & \SI{49}{\newton}, \SI{20}{\radian\per\second} \\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}%
%DIFDELCMD < }
%DIFDELCMD < \end{table}
...
%DIFDELCMD < %%%
\DIFdelend \subsection{Controller, trajectories, and disturbance}
...
the \DIFdelbegin \DIFdel{observability }\DIFdelend \DIFaddbegin \DIFadd{identifiability }\DIFaddend analysis. The external force is a \emph{deterministic} per-axis
...
ablation isolates the effect of the \DIFdelbegin \DIFdel{observability }\DIFdelend \DIFaddbegin \DIFadd{identifiability }\DIFaddend logic, since the two filters
...
\DIFaddbegin \DIFadd{realizations (}\DIFaddend and wind realizations \DIFaddbegin \DIFadd{for the gate runs; the active-excitation runs
are wind-free)}\DIFaddend , shown as mean $\pm$ std; the disturbance-free mass-identification
...
\DIFaddend \subsection{The virtual force sensor}
The accelerometer makes the force algebraic \eqref{eq:regressor}, so the EKF
recovers it without any temporal smoothing. We report \DIFdelbegin \DIFdel{three }\DIFdelend \DIFaddbegin \DIFadd{four }\DIFaddend complementary
...
cleanly. \DIFaddbegin \DIFadd{This is also why the vertical correlation in Table~\ref{tab:id} falls
from \mbox{%DIFAUXCMD
\numrange{0.97}{0.98} }\hskip0pt%DIFAUXCMD
near hover to \mbox{%DIFAUXCMD
\numrange{0.91}{0.95} }\hskip0pt%DIFAUXCMD
at speed while the horizontal
...
rises from \mbox{%DIFAUXCMD
\SI{0.02}{} }\hskip0pt%DIFAUXCMD
to \mbox{%DIFAUXCMD
\SI{14}{\meter\squared\per\second\squared} }\hskip0pt%DIFAUXCMD
in RMS
across the battery), it is uncorrelated with the applied steps, and the vertical
steps are half the size of the horizontal ones (\mbox{%DIFAUXCMD
\SI{0.8}{} }\hskip0pt%DIFAUXCMD
against
\mbox{%DIFAUXCMD
\SI{1.5}{\newton} }\hskip0pt%DIFAUXCMD
on $x$; the vector magnitude is the $\sim$\mbox{%DIFAUXCMD
\SI{2}{\newton} }\hskip0pt%DIFAUXCMD
of
...
\mbox{%DIFAUXCMD
\SI{0.62}{\newton} }\hskip0pt%DIFAUXCMD
in hover, where the mass--force ambiguity biases the plain
filter's $d_z$ (a bias the correlation does not see), to \mbox{%DIFAUXCMD
\SI{0.29}{\newton} }\hskip0pt%DIFAUXCMD
at
\mbox{%DIFAUXCMD
\SI{10.2}{\meter\per\second}}\hskip0pt%DIFAUXCMD
; the dip to \mbox{%DIFAUXCMD
\num{0.91} }\hskip0pt%DIFAUXCMD
at \mbox{%DIFAUXCMD
\SI{6.1}{\meter\per\second}
}\hskip0pt%DIFAUXCMD
is comparable to the run-to-run spread ($\pm0.02$). }\DIFaddend A moving-horizon
...
\DIFdelbegin \DIFdel{Fig.~\ref{fig:dhat} shows the estimate tracking }\DIFdelend \DIFaddbegin \DIFadd{The estimate tracks }\DIFaddend the disturbance in the time domain on all three axes\DIFdelbegin \DIFdel{.
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[t]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=\columnwidth]{figs/fig_calib.png}
%DIFDELCMD < \caption{Calibration curve: estimated force $m\hat{\vect{d}}$ versus applied
%DIFDELCMD < force, per axis, under a sinusoidal wind.}
%DIFDELCMD < \label{fig:calib}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}[t]\color{blue}
\centering
...
accurately\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{: }\DIFaddend Fig.~\ref{fig:massconv} shows the EKF converging from a deliberately
...
\DIFdelbegin \DIFdel{excites the thrust direction}\DIFdelend \DIFaddbegin \DIFadd{disperses the thrust vector}\DIFaddend .
...
filter\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{: }\DIFaddend Fig.~\ref{fig:mass} shows the plain EKF mass against speed under wind
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[t]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=\columnwidth]{figs/fig_mass_id.png}
%DIFDELCMD < \caption{Mass observability against excitation, \emph{ungated} EKF ($N{=}10$ per
%DIFDELCMD < speed, $N{=}9$ in hover; mean $\pm$ std).}
%DIFDELCMD < \label{fig:mass}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}[t]\color{blue}
\centering
...
\DIFaddbegin \DIFadd{The experiment of }\DIFaddend Fig.~\ref{fig:trans} flies an aggressive trajectory followed by a hover, both
...
to the lost \DIFdelbegin \DIFdel{observability }\DIFdelend \DIFaddbegin \DIFadd{identifiability }\DIFaddend and attributes the vertical wind to a mass change; its
...
\DIFdelbegin \DIFdel{observability-aware }\DIFdelend \DIFaddbegin \DIFadd{identifiability-aware }\DIFaddend filter detects $\tilde\sigma$ falling below
...
EKF\DIFdelbegin \DIFdel{(\mbox{%DIFAUXCMD
\SI{7.0}{\percent}}\hskip0pt%DIFAUXCMD
): a }\DIFdelend \DIFaddbegin \DIFadd{: on five repetitions the energy-gated and the plain estimates are identical
at the end of the hover (\mbox{%DIFAUXCMD
\SIrange{6.4}{8.7}{\percent} }\hskip0pt%DIFAUXCMD
drift on a new set of
noise realizations). A }\DIFaddend generic
excitation measure cannot tell hover from a maneuver. Only $\tilde\sigma$, which measures the dispersion that isolates the
mass--vertical-force coupling, detects the \DIFdelbegin \DIFdel{unobservability }\DIFdelend \DIFaddbegin \DIFadd{loss of identifiability }\DIFaddend and protects the
calibration.

\DIFdelbegin %DIFDELCMD < \begin{figure}[t]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=\columnwidth]{figs/fig_trans.png}
%DIFDELCMD < \caption{Mass over a windy maneuver-to-hover transition ($N{=}5$, mean $\pm$
%DIFDELCMD < std); hover shaded, $\tilde\sigma$ and threshold at the bottom.}
%DIFDELCMD < \label{fig:trans}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}[t]\color{blue}
\centering
...
\DIFdelbegin %DIFDELCMD < \subsection{Active excitation recovers an unobservable mass}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsection{Active excitation recovers an unidentifiable mass}
\DIFaddend When the mass is unknown and the vehicle must hover, no passive estimator can
recover it. \DIFaddbegin \DIFadd{The run of }\DIFaddend Fig.~\ref{fig:active} starts from a deliberately wrong prior
(\SI{0.70}{\kilogram}) in \DIFdelbegin \DIFdel{hover}\DIFdelend \DIFaddbegin \DIFadd{a wind-free hover, the same conditions for the passive
and the active filter}\DIFaddend . The passive filter stays frozen at the prior,
...
opens, and the mass converges from \SI{0.70}{} toward the ground truth\DIFdelbegin \DIFdel{, after
which
}\DIFdelend \DIFaddbegin \DIFadd{; after
\mbox{%DIFAUXCMD
\SI{15}{\second} }\hskip0pt%DIFAUXCMD
of gate-open time }\DIFaddend the probe is withdrawn\DIFaddbegin \DIFadd{, the gate closes, }\DIFaddend and the
estimate is \DIFdelbegin \DIFdel{again protected}\DIFdelend \DIFaddbegin \DIFadd{held for the remaining \mbox{%DIFAUXCMD
\SI{34}{\second} }\hskip0pt%DIFAUXCMD
of hover (the slow rise of
$\tilde\sigma$ after withdrawal is the noise-driven thrust jitter of the hover
and stays a factor of two below the threshold)}\DIFaddend . Using the
most informative probe \DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend Section~\ref{sec:probeopt}\DIFdelbegin \DIFdel{)}\DIFdelend , the mass error falls from
\SI{33}{\percent} to \DIFdelbegin \DIFdel{about \mbox{%DIFAUXCMD
\SI{3}{\percent} }\hskip0pt%DIFAUXCMD
($2.5\pm1.6$}\DIFdelend \DIFaddbegin \DIFadd{below \mbox{%DIFAUXCMD
\SI{1}{\percent} }\hskip0pt%DIFAUXCMD
($0.4\pm0.2$}\DIFaddend , $N{=}5$)\DIFdelbegin \DIFdel{; the recovery depends on the probe shape, which the next experiment
optimizes. By contrast, under a }\DIFdelend \DIFaddbegin \DIFadd{, the same
accuracy the filter reaches on the }\DIFaddend disturbance-free \DIFdelbegin \DIFdel{trajectory the same
filter
identifies the mass to within \mbox{%DIFAUXCMD
\SI{0.6}{\percent} }\hskip0pt%DIFAUXCMD
(}\DIFdelend \DIFaddbegin \DIFadd{trajectory of
}\DIFaddend Fig.~\ref{fig:massconv} \DIFdelbegin \DIFdel{); the residual here is the price of identifying in hover from a wrong prior. }\DIFdelend \DIFaddbegin \DIFadd{(\mbox{%DIFAUXCMD
\SI{0.6}{\percent}}\hskip0pt%DIFAUXCMD
): fifteen seconds of probe bring
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[t]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=\columnwidth]{figs/fig_active.png}
%DIFDELCMD < \caption{Mass recovery from a wrong hover prior (\SI{0.70}{\kilogram}; $N{=}5$,
%DIFDELCMD < mean $\pm$ std): the probe (shaded) lifts $\tilde\sigma$, recovers the mass,
%DIFDELCMD < then withdraws.}
%DIFDELCMD < \label{fig:active}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}[!t]\color{blue}
\centering
...
The \DIFaddbegin \DIFadd{speed of the }\DIFaddend recovery in Fig.~\ref{fig:active} depends on the probe shape,
and the identifiability tells us which shape is best: since the Schur complement
is the Fisher information on the mass \DIFdelbegin \DIFdel{\eqref{eq:crb}}\DIFdelend \DIFaddbegin \DIFadd{(Proposition~\ref{prop:crb})}\DIFaddend , the probe
that raises it most per unit deviation tightens the bound \DIFaddbegin \DIFadd{fastest}\DIFaddend . We sweep the
vertical/lateral mix of \eqref{eq:probe} at \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend fixed amplitude budget
\DIFaddbegin \DIFadd{$\rho=\SI{0.8}{\meter}$}\DIFaddend , from a pure lateral tilt to a pure vertical bob, and
measure the \DIFdelbegin \DIFdel{achieved }\DIFdelend $\tilde\sigma$ \DIFdelbegin \DIFdel{and the resulting mass recovery
}\DIFdelend \DIFaddbegin \DIFadd{achieved while the probe is active and the time for the
mass error to settle below \mbox{%DIFAUXCMD
\SI{2}{\percent} }\hskip0pt%DIFAUXCMD
}\DIFaddend (Fig.~\ref{fig:probeopt}, $N{=}5$ per
shape). The \DIFdelbegin \DIFdel{lateral tilt raises }\DIFdelend \DIFaddbegin \DIFadd{achieved }\DIFaddend $\tilde\sigma$ \DIFdelbegin \DIFdel{well above every other mix and is the only shape whose recovery is both accurate and tightly distributed, consistent with \eqref{eq:crb}: it is a variance bound, so
the highest-$\tilde\sigma$ probe admits the lowest-variance recovery. The most informative
mix (}%DIFDELCMD < \probeOptShape%%%
\DIFdel{) recovers the mass }\DIFdelend \DIFaddbegin \DIFadd{rises monotonically from \mbox{%DIFAUXCMD
\num{0.033} }\hskip0pt%DIFAUXCMD
for the
lateral tilt to \mbox{%DIFAUXCMD
\num{0.048} }\hskip0pt%DIFAUXCMD
for the vertical bob, and the settling time falls in
the same order, from $14.3\pm1.3$ }\DIFaddend to \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\SI{\probeOptErr}{\percent}}\hskip0pt%DIFAUXCMD
, against
\mbox{%DIFAUXCMD
\SI{\probeFixErr}{\percent} }\hskip0pt%DIFAUXCMD
for the original mixed shapeat the
same amplitude budget;
the difference is significant against every other shape (}\DIFdelend \DIFaddbegin \DIFadd{$8.3\pm0.6$\,\si{\second} (}\DIFaddend one-sided
\DIFdelbegin \DIFdel{Mann--Whitney, $p<0.01$). This }\DIFdelend \DIFaddbegin \DIFadd{Mann--Whitney against the lateral tilt, $p=0.004$), consistent with
...
mass to \mbox{%DIFAUXCMD
\SIrange{0.4}{1.2}{\percent}}\hskip0pt%DIFAUXCMD
, since the probe is withdrawn only after
\mbox{%DIFAUXCMD
\SI{15}{\second} }\hskip0pt%DIFAUXCMD
of gate-open time whatever its shape. The ranking is the
opposite of the one we obtained with a preliminary \mbox{%DIFAUXCMD
\SI{0.4}{\meter} }\hskip0pt%DIFAUXCMD
budget,
...
\mbox{%DIFAUXCMD
\SI{1}{\hertz} }\hskip0pt%DIFAUXCMD
bob of that amplitude: what counts is the $\tilde\sigma$ the closed
loop actually achieves, not the one the reference prescribes. This is the practical reason for shaping the probe by the
measured $\tilde\sigma$ rather than by a fixed rule, and the }\DIFaddend shape is optimal only
within the two-parameter family we sweep, not the unconstrained optimal input.

\DIFdelbegin %DIFDELCMD < \begin{figure}[t]
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=\columnwidth]{figs/fig_probe_opt.png}
%DIFDELCMD < \caption{Probe shaping: mass recovery error against the vertical/lateral mix at
%DIFDELCMD < a fixed amplitude budget; the $\tilde\sigma$-maximizing mix is circled.}
%DIFDELCMD < \label{fig:probeopt}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}[t]\color{blue}
\centering
...
\SI{1}{\milli\second} \DIFdelbegin \DIFdel{. Fig.~\ref{fig:traj} shows the }\DIFdelend \DIFaddbegin \DIFadd{on the processor of Section~\ref{sec:nmpc}. The }\DIFaddend spatial effect on the figure-eight \DIFaddbegin \DIFadd{is shown in
Fig.~\ref{fig:traj}}\DIFaddend : with the feedforward the vehicle stays on the reference, while
...
\DIFdelbegin %DIFDELCMD < \begin{table}[t]
%DIFDELCMD < \centering
%DIFDELCMD < \caption{EKF estimation against trajectory speed ($N{=}10$; $9$ in hover): mass estimate and
%DIFDELCMD < per-axis force correlation with the ground truth. The force is read accurately
%DIFDELCMD < throughout; the (ungated) mass is biased and noisy in hover and accurate under
%DIFDELCMD < excitation.}
%DIFDELCMD < \label{tab:id}
%DIFDELCMD < \begin{tabular}{cccc}
%DIFDELCMD < \toprule
%DIFDELCMD < Peak speed [\si{\meter\per\second}] & $\hat m$ [\si{\kilogram}] & corr $d_x/d_y$ & corr $d_z$ \\
%DIFDELCMD < \midrule
%DIFDELCMD < hover & $1.101\pm0.035$ & $0.98/0.98$ & $0.97$ \\
%DIFDELCMD < 2.1   & $1.083\pm0.027$ & $0.97/0.98$ & $0.98$ \\
%DIFDELCMD < 3.8   & $1.049\pm0.028$ & $0.97/0.98$ & $0.95$ \\
%DIFDELCMD < 6.1   & $1.051\pm0.010$ & $0.97/0.97$ & $0.91$ \\
%DIFDELCMD < 8.2   & $1.056\pm0.003$ & $0.96/0.95$ & $0.94$ \\
%DIFDELCMD < 10.2  & $1.053\pm0.002$ & $0.95/0.94$ & $0.93$ \\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \end{table}
%DIFDELCMD < 
...
$\tilde\sigma$ of \eqref{eq:snorm} removes that dependence: \DIFdelbegin \DIFdel{settled }\DIFdelend hover sits at
\DIFdelbegin \DIFdel{$\tilde\sigma\approx0.005$ (windowed, peak $\approx0.007$) while an aggressive
trajectory
reaches $\tilde\sigma\approx0.34$,
nearly }\DIFdelend \DIFaddbegin \DIFadd{$\tilde\sigma\approx0.002$ (steady hover, still or windy: Fig.~\ref{fig:active}
and Table~\ref{tab:info}) to \mbox{%DIFAUXCMD
\num{0.005} }\hskip0pt%DIFAUXCMD
(the post-maneuver windy hover of
Fig.~\ref{fig:trans}, \mbox{%DIFAUXCMD
\SI{1}{\second} }\hskip0pt%DIFAUXCMD
window median) while the fastest trajectory
reaches \mbox{%DIFAUXCMD
\num{0.46} }\hskip0pt%DIFAUXCMD
(Table~\ref{tab:info}),
}\DIFaddend two orders of magnitude apart. A single $\tilde\sigma_{\min}=0.009$ classifies
hover, maneuver, and the active probe alike: the probe lifts \DIFdelbegin \DIFdel{the instantaneous }\DIFdelend $\tilde\sigma$ \DIFdelbegin \DIFdel{above the threshold often
enough to
re-open the gate (Fig.~\ref{fig:probeopt} reports the run median, which
sits just below it, with the lateral tilt the highest of the family}\DIFdelend \DIFaddbegin \DIFadd{to
\mbox{%DIFAUXCMD
\numrange{0.033}{0.048}}\hskip0pt%DIFAUXCMD
, four to five times the threshold (Table~\ref{tab:info}}\DIFaddend ),
while hover keeps the gate closed. \DIFdelbegin \DIFdel{The remaining sensitivity is to transient gusts during a windy hover transition,
which can momentarily spike $\tilde\sigma$; the dwell time $t_d$, which requires the
thresholdto be exceeded for a sustained interval, rejects these so the gate holds.
The threshold is thus dimensionless and shared across all regimes, with }\DIFdelend \DIFaddbegin \DIFadd{How far the threshold, }\DIFaddend the \DIFaddbegin \DIFadd{window, the }\DIFaddend dwell, \DIFdelbegin \DIFdel{not the
absolute signal scale, handling the residual gust sensitivity}\DIFdelend \DIFaddbegin \DIFadd{the
process and measurement noise, and the maneuver intensity can move before the
gate degrades is quantified in Section~\ref{sec:sens}}\DIFaddend .
...
The \DIFaddbegin \DIFadd{ranges are chosen to bracket what a real platform presents before any
...
wear, which is of the order of \mbox{%DIFAUXCMD
\SI{10}{\percent}}\hskip0pt%DIFAUXCMD
.
The }\DIFaddend sensor degrades gracefully and proportionally, with no breakdown. The force
...
near-linear degradation \DIFdelbegin \DIFdel{indicates the calibration error is recoverable by }\DIFdelend \DIFaddbegin \DIFadd{suggests that }\DIFaddend a one-time bench calibration of the bias
and scale \DIFaddbegin \DIFadd{may remove much of it, which we have not tested}\DIFaddend .
...
\DIFaddbegin \subsection{Fisher information, threshold, and gate sensitivity}\label{sec:sens}
\DIFadd{Proposition~\ref{prop:crb} bounds the variance of the mass estimate; we check it
...
window is \mbox{%DIFAUXCMD
\SI{12}{\percent} }\hskip0pt%DIFAUXCMD
of the constant-force value at the
implemented $q_d$ (Fig.~\ref{fig:fisher}(b)). Under excitation (\mbox{%DIFAUXCMD
\SIrange{6.1}{10.2}{\meter\per\second}}\hskip0pt%DIFAUXCMD
)
the empirical spread falls from \mbox{%DIFAUXCMD
\SI{10}{} }\hskip0pt%DIFAUXCMD
to \mbox{%DIFAUXCMD
\SI{2.2}{\gram} }\hskip0pt%DIFAUXCMD
while the
constant-force bound falls from \mbox{%DIFAUXCMD
\SI{2.4}{} }\hskip0pt%DIFAUXCMD
to \mbox{%DIFAUXCMD
\SI{0.9}{\gram}}\hskip0pt%DIFAUXCMD
; scaling by the
random-walk ratio of Fig.~\ref{fig:fisher}(b) brings the bound to
\mbox{%DIFAUXCMD
\SIrange{2.5}{7}{\gram}}\hskip0pt%DIFAUXCMD
, the scale of the spread (it can even exceed it at the two
...
(\mbox{%DIFAUXCMD
\SI{35}{\gram}}\hskip0pt%DIFAUXCMD
) exceeds both bounds by a wide margin because the error there is a
}\emph{\DIFadd{bias}}\DIFadd{, the vertical wind misread as mass, which no variance bound captures:
...
\DIFadd{Table~\ref{tab:info} separates the two roles of the identifiability signal. The
...
flight between \mbox{%DIFAUXCMD
\SI{3.8}{} }\hskip0pt%DIFAUXCMD
and \mbox{%DIFAUXCMD
\SI{6.1}{\meter\per\second}}\hskip0pt%DIFAUXCMD
; because all the
...
\DIFadd{The window length and the threshold are coupled, and we quantify the coupling
...
\mbox{%DIFAUXCMD
\SIrange{0.25}{4}{\second}}\hskip0pt%DIFAUXCMD
. The ratio between the maneuver and the hover medians
of $\tilde\sigma$ is \mbox{%DIFAUXCMD
\num{5} }\hskip0pt%DIFAUXCMD
at \mbox{%DIFAUXCMD
\SI{0.25}{\second}}\hskip0pt%DIFAUXCMD
, \mbox{%DIFAUXCMD
\num{15} }\hskip0pt%DIFAUXCMD
at
\mbox{%DIFAUXCMD
\SI{0.5}{\second}}\hskip0pt%DIFAUXCMD
, peaks at \mbox{%DIFAUXCMD
\num{28} }\hskip0pt%DIFAUXCMD
at the \mbox{%DIFAUXCMD
\SI{1}{\second} }\hskip0pt%DIFAUXCMD
window used, and
falls to \mbox{%DIFAUXCMD
\num{16} }\hskip0pt%DIFAUXCMD
and \mbox{%DIFAUXCMD
\num{9} }\hskip0pt%DIFAUXCMD
at \mbox{%DIFAUXCMD
\SI{2}{} }\hskip0pt%DIFAUXCMD
and \mbox{%DIFAUXCMD
\SI{4}{\second}}\hskip0pt%DIFAUXCMD
, because a short
window sees too little of the maneuver and a long one lets the windy hover
accumulate dispersion (its median $\tilde\sigma$ rises from \mbox{%DIFAUXCMD
\num{0.0025} }\hskip0pt%DIFAUXCMD
at
\mbox{%DIFAUXCMD
\SI{0.25}{\second} }\hskip0pt%DIFAUXCMD
to \mbox{%DIFAUXCMD
...
\mbox{%DIFAUXCMD
\num{0.009} }\hskip0pt%DIFAUXCMD
therefore sits between the two medians, with a margin of about two
on the hover side and more than ten on the maneuver side for windows of
\mbox{%DIFAUXCMD
\SIrange{0.5}{1}{\second}}\hskip0pt%DIFAUXCMD
, and would
have to be raised for windows of \mbox{%DIFAUXCMD
\SI{2}{\second} }\hskip0pt%DIFAUXCMD
or longer. Within any window the
windy hover still produces transient spikes (its 95th percentile at
\mbox{%DIFAUXCMD
\SI{1}{\second} }\hskip0pt%DIFAUXCMD
is \mbox{%DIFAUXCMD
\num{0.027}}\hskip0pt%DIFAUXCMD
), which is what the dwell rejects.
}

\DIFadd{Table~\ref{tab:sens} reports the sensitivity study proper. Each row repeats
...
end-of-transition values of \mbox{%DIFAUXCMD
\SI{3.3}{} }\hskip0pt%DIFAUXCMD
and \mbox{%DIFAUXCMD
\SI{7.0}{\percent} }\hskip0pt%DIFAUXCMD
of
Fig.~\ref{fig:trans}). }\emph{\DIFadd{Threshold}}\DIFadd{: below \mbox{%DIFAUXCMD
\num{0.009} }\hskip0pt%DIFAUXCMD
the
gate re-opens on the windy-hover excursions and its advantage shrinks
(\mbox{%DIFAUXCMD
\SIrange{3.4}{3.8}{\percent}}\hskip0pt%DIFAUXCMD
); at \mbox{%DIFAUXCMD
\num{0.015}}\hskip0pt%DIFAUXCMD
, which sits at the level of those
excursions, it opens intermittently with the frozen covariance and becomes
erratic ($5.1\pm2.8$\,\si{\percent}); at \mbox{%DIFAUXCMD
\numrange{0.03}{0.06} }\hskip0pt%DIFAUXCMD
it stays shut
through the whole hover and holds the mass to \mbox{%DIFAUXCMD
\SIrange{0.4}{0.6}{\percent}}\hskip0pt%DIFAUXCMD
. The
...
\mbox{%DIFAUXCMD
\SI{0.80}{\meter} }\hskip0pt%DIFAUXCMD
budget the probe reaches $\tilde\sigma\approx0.048$
(Table~\ref{tab:info}; a preliminary \mbox{%DIFAUXCMD
\SI{0.40}{\meter} }\hskip0pt%DIFAUXCMD
budget left it at
the threshold, Section~\ref{sec:probeopt}); with it a threshold of \mbox{%DIFAUXCMD
\num{0.03} }\hskip0pt%DIFAUXCMD
would be admissible for the probe and would hold
the mass below \mbox{%DIFAUXCMD
\SI{1}{\percent}}\hskip0pt%DIFAUXCMD
, but it would exclude the
\mbox{%DIFAUXCMD
\SI{3.8}{\meter\per\second} }\hskip0pt%DIFAUXCMD
flight ($\tilde\sigma=0.014$) from identification, so
we retain \mbox{%DIFAUXCMD
\num{0.009} }\hskip0pt%DIFAUXCMD
and accept the residual re-openings in windy hover.
}\emph{\DIFadd{Dwell}}\DIFadd{: the gate is never worse than the plain filter for dwells of
\mbox{%DIFAUXCMD
\SIrange{0.1}{1}{\second} }\hskip0pt%DIFAUXCMD
and is best at the \mbox{%DIFAUXCMD
\SI{0.25}{\second} }\hskip0pt%DIFAUXCMD
used; a short
...
end-of-hover value at \mbox{%DIFAUXCMD
\SI{1}{\second}}\hskip0pt%DIFAUXCMD
, not listed in Table~\ref{tab:sens}, scatters
by $\pm7$\,\si{\percent}).
}\emph{\DIFadd{Window}}\DIFadd{: \mbox{%DIFAUXCMD
\SI{0.5}{\second} }\hskip0pt%DIFAUXCMD
is too short to separate the regimes and the
gate degrades to \mbox{%DIFAUXCMD
\SI{8.8}{\percent}}\hskip0pt%DIFAUXCMD
; \mbox{%DIFAUXCMD
\SIrange{1}{2}{\second} }\hskip0pt%DIFAUXCMD
hold at
\mbox{%DIFAUXCMD
\SIrange{2.2}{3.1}{\percent}}\hskip0pt%DIFAUXCMD
. }\emph{\DIFadd{Process noise}}\DIFadd{: the gate helps for
...
is already at \mbox{%DIFAUXCMD
\SI{3.6}{\percent}}\hskip0pt%DIFAUXCMD
. The force process noise sets how much of the
...
\mbox{%DIFAUXCMD
\SI{53}{\percent}}\hskip0pt%DIFAUXCMD
, which the gate cuts to \mbox{%DIFAUXCMD
\SI{6.9}{\percent}}\hskip0pt%DIFAUXCMD
; at $q_d=10^{-1}$
the force absorbs everything, the plain filter drifts by only
\mbox{%DIFAUXCMD
\SI{1.8}{\percent}}\hskip0pt%DIFAUXCMD
, and the gate is unnecessary ($4.7\pm2.6$\,\si{\percent}). A
...
reading. }\emph{\DIFadd{Accelerometer noise}}\DIFadd{: added white noise of
\mbox{%DIFAUXCMD
\SIrange{0.3}{0.7}{\meter\per\square\second} }\hskip0pt%DIFAUXCMD
leaves the gated error at
\mbox{%DIFAUXCMD
\SIrange{2.2}{2.5}{\percent}}\hskip0pt%DIFAUXCMD
; the plain filter's own drift shrinks
(\mbox{%DIFAUXCMD
\SIrange{1.9}{2.9}{\percent}}\hskip0pt%DIFAUXCMD
) because a noisier residual moves the mass less,
so the margin narrows; at \mbox{%DIFAUXCMD
\SI{0.3}{\meter\per\square\second} }\hskip0pt%DIFAUXCMD
the two are
statistically indistinguishable ($2.5\pm1.1$ against $1.9\pm0.9$). }\emph{\DIFadd{Maneuver intensity}}\DIFadd{: at peak speeds of
\mbox{%DIFAUXCMD
\SIrange{5.9}{8.4}{\meter\per\second} }\hskip0pt%DIFAUXCMD
the picture is unchanged
(\mbox{%DIFAUXCMD
\SIrange{2.2}{2.7}{} }\hskip0pt%DIFAUXCMD
against \mbox{%DIFAUXCMD
...
vulnerable to the transient at the hover onset (\mbox{%DIFAUXCMD
\SI{44}{\percent} }\hskip0pt%DIFAUXCMD
plain,
...
the gate is therefore a window of \mbox{%DIFAUXCMD
\SIrange{1}{2}{\second}}\hskip0pt%DIFAUXCMD
, a dwell of about
\mbox{%DIFAUXCMD
\SI{0.25}{\second} }\hskip0pt%DIFAUXCMD
(longer dwells keep the RMS error but make the end value
...
\DIFaddend \subsection{Estimator comparison}
...
and the window adds cost without recovering either. Our \DIFdelbegin \DIFdel{observability-aware }\DIFdelend \DIFaddbegin \DIFadd{identifiability-aware }\DIFaddend EKF
...
\DIFdelbegin %DIFDELCMD < \begin{table}[t]
%DIFDELCMD < \centering
%DIFDELCMD < \caption{Estimator comparison on identical data. Force entries are pooled over
%DIFDELCMD < the battery; the excited-mass entries pool the well-excited runs
%DIFDELCMD < (8.2--10.2\,\si{\meter\per\second}, $N{=}10$ each speed); hover and recovery
%DIFDELCMD < entries are from Figs.~\ref{fig:trans} and~\ref{fig:active}.}
%DIFDELCMD < \label{tab:compare}
%DIFDELCMD < \begin{tabular}{lccc}
%DIFDELCMD < \toprule
%DIFDELCMD <  & MHE & plain EKF & obs-aware EKF \\
%DIFDELCMD <  &     & (baseline) & (ours) \\
%DIFDELCMD < \midrule
%DIFDELCMD < horizontal force corr      & 0.96 & 0.97 & 0.97 \\
%DIFDELCMD < vertical force corr        & 0.66 & 0.95 & 0.95 \\
%DIFDELCMD < mass error, excited        & $+0.4\%$ & $+0.4\%$ & $+0.4\%$ \\
%DIFDELCMD < mass error, windy hover    & $5.3\%$ & $7.0\%$ & $\mathbf{3.3\%}$ \\
%DIFDELCMD < recover mass, wrong prior  & no & no & yes \\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
...
the battery; the mass entries under excitation pool the well-excited runs
...
parameter, the mass, is \DIFdelbegin \DIFdel{observable }\DIFdelend \DIFaddbegin \DIFadd{identifiable }\DIFaddend only under excitation and is confounded with a
vertical force in hover. We made the calibration \DIFdelbegin \DIFdel{observability-aware }\DIFdelend \DIFaddbegin \DIFadd{identifiability-aware }\DIFaddend on two levels:
...
the contribution therefore lies in the \DIFdelbegin \DIFdel{observability }\DIFdelend \DIFaddbegin \DIFadd{identifiability }\DIFaddend logic, not in the estimator
...
choice of the most informative excitation. The \DIFaddbegin \DIFadd{sensitivity study bounds where this holds: the gate helps for windows of
\mbox{%DIFAUXCMD
\SIrange{1}{2}{\second}}\hskip0pt%DIFAUXCMD
, a dwell of about \mbox{%DIFAUXCMD
\SI{0.25}{\second}}\hskip0pt%DIFAUXCMD
, a mass process
...
measured rather than prescribed. The }\DIFaddend main
...
is \DIFdelbegin \DIFdel{a continuous (rather than family-restricted) }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend optimal-input \DIFdelbegin \DIFdel{design}\DIFdelend \DIFaddbegin \DIFadd{design over continuous inputs rather than over a probe family}\DIFaddend .
Because the filter reads any external specific force, the approach extends toward
contact and tether forces, where an \DIFdelbegin \DIFdel{observability-aware }\DIFdelend \DIFaddbegin \DIFadd{identifiability-aware }\DIFaddend wrench estimate would
support aerial physical interaction~\cite{ollero2022past,augugliaro2013admittance} without a dedicated force sensor.

\section*{Code Availability}
The source code and experiment scripts are openly available at \DIFdelbegin \href{https://github.com/bryansgue/observability-aware}{\DIFdel{GITHUB}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \url{https://github.com/josevarela90/observability-aware}\DIFaddend .
