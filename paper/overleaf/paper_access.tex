...
parameter it cannot see. Figure~\ref{fig:system} shows the resulting loop and the role of the
...
$\sigma/r_s$ is the \emph{information content} that bounds the estimate;
...
\SI{2.4}{\percent} after ten. The bound is a floor, not a guarantee, but it
states what the threshold admits: a window below it could not resolve the mass,
even with an efficient estimator, to better than the \SIrange{5}{7}{\percent}
hover bias the gate guards against (Section~\ref{sec:results}), so letting it
update the mass could only trade one error for a larger one; and at least ten
seconds of admissible data are needed before the floor falls below the
\SI{3.3}{\percent} residual the gate leaves in the windy hover
(Section~\ref{sec:results}). The estimator uses
...
identifiable throughout, and the mass enters only as the multiplicative thrust
scaling $\tfrac{\hat m}{m}$, whose error is bounded by \eqref{eq:crb} where the mass is
identifiable and frozen at its identification value where it is not. A formal
...
Figure~\ref{fig:dhat} shows the estimate tracking the disturbance in the time
...
accurately. Figure~\ref{fig:massconv} shows the EKF converging from a deliberately
...
filter. Figure~\ref{fig:mass} shows the plain EKF mass against speed under wind
...
Figure~\ref{fig:trans} flies an aggressive trajectory followed by a hover, both
...
Figure~\ref{fig:mass} (\SI{4.8}{\percent}), because the maneuver leaves the filter
...
recover it. Figure~\ref{fig:active} starts from a deliberately wrong prior
...
Figure~\ref{fig:massconv} (\SI{0.6}{\percent}): fifteen seconds of probe bring
...
\SI{1}{\milli\second} on the laptop-class processor of Section~\ref{sec:nmpc}. Figure~\ref{fig:traj} shows the spatial effect on the
...
and scale would remove most of it, which we have not tested.
...
so the margin narrows. \emph{Maneuver intensity}: at peak speeds of
...
established by one simulated platform; \eqref{eq:thrcrb} gives the rule to
re-derive the threshold from $N$, $r_s$, and the mass uncertainty that can be
tolerated.
...
The source code and experiment scripts are openly available at \url{https://github.com/bryansgue/observability-aware}.
